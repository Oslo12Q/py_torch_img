...
class Mytest(data.Dataset):
    ...
    def __init__(self, image_dir,  transform, config):
...
def get_loader(image_dir, attr_path, selected_attrs, config, crop_size=178, image_size=128,
    ...
    if dataset == 'CelebA':
        ...
        dataset = Mytest(image_dir, transform, config)
...
def get_loader1(image_dir,  config, dataset, num_workers=1):
    """Build and return a data loader."""
    transform = []
    transform.append(T.ToTensor())
    transform.append(T.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)))
